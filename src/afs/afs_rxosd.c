...
getRxosdConn(struct rxosd_Variables *v, struct osd_obj *o,
             struct server **ts, struct afs_conn **aconn,
	     struct rx_connection **arxconn)
{
    afs_uint32 ip;
    afs_uint16 port;
    afs_int32 code = 0, service = 0;

    if (o->addr.ip.addrtype == RX_ADDRTYPE_IPV4) {
        memcpy(&ip, o->addr.ip.addr.addr_val, 4);
        port = o->addr.port;
	port = htons(port);
        service = o->addr.service;
    } else {
        afs_warn("getRxosdConn: found IPv6 addr, not yet supported\n");
        code = EIO;
        return code;
    }
    *ts = afs_GetServer(&ip, 1, v->avc->f.fid.Cell, port,
    ...
    if (cryptall) {
        ...
        if (service)
            *aconn = afs_ConnBySAsrv((*ts)->addr, port, service, v->avc->f.fid.Cell,
                           tu, 1, 1, SHARED_LOCK, 0, arxconn);
        else
            *aconn = afs_ConnBySA((*ts)->addr, port, v->avc->f.fid.Cell,
                           tu, 1, 1, SHARED_LOCK, 0, arxconn);
        afs_PutUser(tu, SHARED_LOCK);
    } else {
        if (service)
            *aconn = afs_ConnBySAsrv((*ts)->addr, port, service, v->avc->f.fid.Cell,
                    (struct unixuser *)&dummyuser, 1, 1, SHARED_LOCK, 0, arxconn);
        else
            *aconn = afs_ConnBySA((*ts)->addr, port, v->avc->f.fid.Cell,
                    (struct unixuser *)&dummyuser, 1, 1, SHARED_LOCK, 0, arxconn);
    }
...
afs_int32
rxosd_Destroy(void **r, afs_int32 code)
{
    afs_int32 i, j;
    afs_int32 busy, code2;
    ...
    *r = NULL;
	...
	RX_AFS_GUNLOCK();
	if (v->writing) {
	    for (code2 = RX_CALL_BUSY, busy = 0; code2 == RX_CALL_BUSY; busy++)
	        code2 = RXAFSOSD_EndAsyncStore(v->rxconn, &v->avc->f.fid.Fid, 
				v->transid, v->avc->f.m.Length, 0, 0, 0,
				code, &v->aE, &InStatus, &v->OutStatus);
	    if ((code2 && debug_rxosd) || (debug_rxosd && busy > 1)
              || (debug_rxosd > 1 && busy))
                afs_warn("RXAFS_EndAsyncStore rc %d had %u rx busy calls\n",
                         code2, busy);
	    if (v->aE.error == 1) {
	    ...
	    for (code2 = RX_CALL_BUSY, busy = 0; code2 == RX_CALL_BUSY; busy++)
	        code2 = RXAFSOSD_EndAsyncFetch(v->rxconn, &v->avc->f.fid.Fid, 
					v->transid, 0, 0);
	    if ((code2 && debug_rxosd) || (debug_rxosd && busy > 1)
              || (debug_rxosd > 1 && busy))
                afs_warn("RXAFS_EndAsyncFetch rc %d had %u rx busy calls\n",
			 code2, busy);
	}
        RX_AFS_GLOCK();
    }
    ...
    if (v->fs_conn)
	afs_PutConn(v->fs_conn, v->rxconn, SHARED_LOCK);
...
start_store(struct rxosd_Variables *v, afs_uint64 offset)
{
    ...
    for (m=0; m<v->copies; m++) {
	    ...
	    afs_PutConn(tc, rxconn, SHARED_LOCK);
	        ...
	        code = rx_EndCall(v->call[lc], code);
	        v->call[lc] = NULL;
	        code = EIO;
		goto bad;
            }
...
handleError(struct rxosd_Variables *v, afs_int32 i, afs_int32 error)
{
	...
	short port = 0;
	if (v->call[i]) {
	    host = rx_HostOf(rx_PeerOf(rx_ConnectionOf(v->call[i])));
	    port = rx_PortOf(rx_PeerOf(rx_ConnectionOf(v->call[i])));
	} else
	    printf("handleError: v->call[%d] == 0\n", i);
        RX_AFS_GLOCK();
        ts = afs_GetServer(
			&host, 1, v->avc->f.fid.Cell, port,
			WRITE_LOCK, (afsUUID *)0, 0, NULL);
        ...
        afs_ServerDown(ts->addr, error, rx_ConnectionOf(v->call[i]));	
...
{
    afs_int32 code = 0, maxcode = 0, i, j, k, L, busy;
    afs_uint32 tlen;
    struct rxosd_Variables *v = (struct rxosd_Variables *)r;
    afs_int32 error = 0;
    ...
    *byteswritten = 0;
	...
	/* extend expiration for async store request */	
 	RX_AFS_GUNLOCK();
	for (code = RX_CALL_BUSY, busy = 0; code == RX_CALL_BUSY; busy++)
	    code = RXAFSOSD_ExtendAsyncStore(v->rxconn, &v->avc->f.fid.Fid,
				 v->transid, &v->expires);
	if ((code && debug_rxosd) || (debug_rxosd && busy > 1)
	  || (debug_rxosd > 1 && busy))
	    afs_warn("RXAFSOSD_ExtendAsyncStore rc %d had %u rx busy calls\n",
		     code, busy);
	RX_AFS_GLOCK();
	if (code) 
	    return EIO;
	...
	afs_int32 code1;
        RX_AFS_GUNLOCK();
	for (code1 = RX_CALL_BUSY, busy = 0; code1 == RX_CALL_BUSY; busy++)
	    code1 = RXAFSOSD_CheckOSDconns(v->rxconn);
	if ((code1 && debug_rxosd) || (debug_rxosd && busy > 1)
	  || (debug_rxosd > 1 && busy))
	    afs_warn("RXAFSOSD_CheckOSDconns rc %d had %u rx busy calls\n",
		     code1, busy);
        RX_AFS_GLOCK();
    }
    return code;
...
rxosd_storeClose(void *r, struct AFSFetchStatus *OutStatus, int *doProcessFS)
{
    ...
    for (j=0; j<v->copies; j++) {
	    ...
	    int k = i + j * v->stripes;
	    if (v->call[k]) {
		struct rx_connection *rxconn = rx_ConnectionOf(v->call[k]);
		if (!v->ometaP[k])
		    v->ometaP[k] = &out;
    	        RX_AFS_GUNLOCK();
    ...
    *doProcessFS = 0;
	...
	RX_AFS_GUNLOCK();
	for (code = RX_CALL_BUSY, busy = 0; code == RX_CALL_BUSY; busy++)
	    code = RXAFSOSD_EndAsyncStore(v->rxconn, &v->avc->f.fid.Fid, 
				v->transid, v->avc->f.m.Length, 0, 0, 0,
				0, &v->aE, &InStatus, OutStatus);
	if ((code && debug_rxosd) || (debug_rxosd && busy > 1)
	  || (debug_rxosd > 1 && busy))
	    afs_warn("RXAFSOSD_EndAsyncStore RC %d had %u rx busy calls\n",
		     code, busy);
	RX_AFS_GLOCK();
        if (!code) {
            *doProcessFS = 1;
		...
		struct storeOps **ops, void **rock)
{
    ...
    v->fs_conn = afs_ConnSrv(&avc->f.fid, areq, VICEDOSD_SERVICE, SHARED_LOCK,
			      &v->rxconn);
    if (!v->fs_conn) {
	code = EIO;
	goto bad;
    ...
    while (1) {
        RX_AFS_GUNLOCK();
        v->a.type = listType;
	v->a.async_u.p1.osd_fileList_len = 0;
	v->a.async_u.p1.osd_fileList_val = NULL;
	p.type = 6;
	...
	startTime = osi_Time();
	for (code = RX_CALL_BUSY, busy = 0; code == RX_CALL_BUSY; busy++)
	    code = RXAFSOSD_StartAsyncStore(v->rxconn,
				 (struct AFSFid *) &avc->f.fid.Fid,
				&p, &v->a, &v->maxlength, &v->transid, &v->expires, 
				&v->OutStatus);
	if ((code && debug_rxosd) || (debug_rxosd && busy > 1)
	  || (debug_rxosd > 1 && busy))
	    afs_warn("RXAFS_StartAsyncStore rc %d had %u rx busy calls\n",
		     code, busy);
        RX_AFS_GLOCK();
	...
	if (waitcount == 0) {
	    afs_uint32 host = 0;
	    if (v->rxconn)
	        host = ntohl(rx_HostOf(rx_PeerOf(v->rxconn)));
	    else
		printf("v->rxconn == 0\n");
	    if (code == VBUSY)
    ...
    v->osd_file = v->a.async_u.p1.osd_fileList_val;
...
retry2:
		...
		if (!alive || code) {
		    ...
		    if (tc && tc->id)
			    ...
			    rx_HostOf(rx_PeerOf(tc->id)), code);
		    else
			printf("start_fetch: tc->id == NULL\n");
            	    afs_Trace3(afs_iclSetp, CM_TRACE_WASHERE,
                   	ICL_TYPE_STRING, __FILE__,
                   	ICL_TYPE_INT32, __LINE__, ICL_TYPE_INT32, code);
		    if (code < 0) {
			afs_ServerDown(ts->addr, code, rx_ConnectionOf(v->call[k]));
		   	if (!afs_soft_mounted)
		    	    sawDown = 1;
		    }
		    v->call[k] = 0;
...
rxosd_fetchRead(void *r, afs_uint32 length, afs_uint32 *bytesread)
{
    ...
    *bytesread = 0;
	...
	RX_AFS_GUNLOCK();
	for (code = RX_CALL_BUSY, busy = 0; code == RX_CALL_BUSY; busy++)
	    code = RXAFSOSD_ExtendAsyncFetch(v->rxconn, &v->avc->f.fid.Fid,
				 v->transid, &v->expires);
	if ((code && debug_rxosd) || (debug_rxosd && busy > 1)
	  || (debug_rxosd > 1 && busy))
	    afs_warn("RXAFSOSD_ExtendAsyncFetch rc %d had %u rx busy clla\n",
	  	     code, busy);
	RX_AFS_GLOCK();
	if (code) 
	    return EIO;
	...
	*bytesread += L;
	...
	afs_int32 code1;
        RX_AFS_GUNLOCK();
	for (code1 = RX_CALL_BUSY, busy = 0; code1 == RX_CALL_BUSY; busy++)
	    RXAFSOSD_CheckOSDconns(v->rxconn);
	if ((code1 && debug_rxosd) || (debug_rxosd && busy > 1)
	  || (debug_rxosd > 1 && busy))
	    afs_warn("RXAFSOSD_CheckOSDconns rc %d had %u rx busy clla\n",
		     code1, busy);
        RX_AFS_GLOCK();
    }
    return code;
...
{
    afs_int32 i, k, avail, code = 0, busy;
...
afs_int32
rxosd_fetchInit(struct vcache *avc, afs_offs_t base,
		...
		struct fetchOps **ops, void **rock)
{
	...
	v->a.async_u.p1.osd_fileList_val = NULL;
        RX_AFS_GUNLOCK();
	for (code = RX_CALL_BUSY, busy = 0; code == RX_CALL_BUSY; busy++)
	    code = RXAFSOSD_StartAsyncFetch(v->rxconn,
				(struct AFSFid *) &avc->f.fid.Fid,
				&p, &v->a, &v->transid, &v->expires,
		 		&v->OutStatus, &v->CallBack);
	if ((code && debug_rxosd) || (debug_rxosd && busy > 1)
	  || (debug_rxosd > 1 && busy))
	    afs_warn("RXAFSOSD_StartAsyncFetch rc %d had %u rx busy calls\n",
		     code, busy);
        RX_AFS_GLOCK();
	if (code != OSD_WAIT_FOR_TAPE && code != VBUSY && code != VRESTARTING) 
	    break;
	if (waitcount == 0) {
		...
		printf("auch hier v->rxconn == NULL\n");
	    if (code == VBUSY)
	        afs_warn("waiting for busy volume %u\n", avc->f.fid.Fid.Volume);
	    else if (code == VRESTARTING)
	        afs_warn("waiting for restarting server %u.%u.%u.%u\n", 
...
void 
rxosd_checkProtocol(struct vcache *avc, struct vrequest *areq, afs_size_t length)
{
    afs_int32 code, busy;
    ...
    if ((afs_protocols & RX_OSD) && (avc->protocol & POSSIBLY_OSD)) {
        tc = afs_ConnSrv(&avc->f.fid, areq, VICEDOSD_SERVICE, SHARED_LOCK, &rxconn);
        if (tc) {
	    RX_AFS_GUNLOCK();
	    for (code = RX_CALL_BUSY, busy = 0; code == RX_CALL_BUSY; busy++)
	        code = RXAFSOSD_ApplyOsdPolicy(rxconn, &avc->f.fid.Fid, length,
				           &protocol);
	    if ((code && debug_rxosd) || (debug_rxosd && busy > 1)
	      || (debug_rxosd > 1 && busy))
		afs_warn("RXAFSOSD_ApplyOsdPolicy rc %d had %u rx busy calls\n",
			 code, busy);
	    RX_AFS_GLOCK();
	    if (!code) {
...
rxosd_bringOnline(struct vcache *avc, struct vrequest *areq)
{
	...
	v->a.type = 1;
        RX_AFS_GUNLOCK();
	for (code = RX_CALL_BUSY, busy = 0; code == RX_CALL_BUSY; busy++)
            code = RXAFSOSD_BringOnline(v->rxconn,
				(struct AFSFid *) &avc->f.fid.Fid,
                                &v->OutStatus, &v->CallBack);
	if ((code && debug_rxosd) || (debug_rxosd && busy > 1)
	  || (debug_rxosd > 1 && busy))
	    afs_warn("RXAFSOSD_BringOnline rc %d had %u rx busy calls\n",
		     code, busy);
        RX_AFS_GLOCK();
	if (code != OSD_WAIT_FOR_TAPE && code != VBUSY && code != VRESTARTING) 
	    break;
	if (waitcount == 0) {
		...
		printf("bringonline: v->rxconn == NULL\n");
	    if (code == VBUSY)
...
afs_int32
set_afs_protocol(afs_uint32 mask_in, afs_uint32 *mask_out)
{
    afs_uint32 mask = 0;
    ...
    if (mask_in) {
        ...
        if (mask_in & DEBUG_RXOSD)
            debug_rxosd++;
        else
            debug_rxosd = 0;

        if (mask_in & RX_OSD_SOFT)
            afs_soft_mounted = 1;
        else
            afs_soft_mounted = 0;

        if (mask_in & NO_HSM_RECALL)
...
init_rxosd_support()
{
    osd_procs->rxosd_fetchInit = rxosd_fetchInit;
    osd_procs->rxosd_storeInit = rxosd_storeInit;
    osd_procs->rxosd_checkProtocol = rxosd_checkProtocol;
    osd_procs->rxosd_bringOnline = rxosd_bringOnline;
    osd_procs->set_afs_protocol = set_afs_protocol;
    afs_protocols |= RX_OSD;
    printf("rxosd support initialized\n");
}
